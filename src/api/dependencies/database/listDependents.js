...
export async function listDependents(
  ...
  metrics.counter('ListDependentsOffset', offset)
  metrics.counter('ListDependentsFilterCount', keys.length)
  ...
  const sql = `
    SELECT _total, name, version, depversion, environments, teams, tags, stage FROM
    (
      SELECT COUNT(*) OVER() AS _total, e.name, e.version, d.version AS depversion, e.created_at, e.stage,
      ...
      LEFT JOIN tags AS env ON env.entity_name = e.name AND env.entity_version = e.version AND env.value IN (${environmentTags.map((tag) => `'${tag}'`).join(',')})
      WHERE ${where.join(' AND ')}
      GROUP BY e.name, e.version, d.version, e.created_at, e.stage
    ) AS r
    ORDER BY r.name ASC, r.created_at DESC, r.version DESC
    LIMIT ${limit}
    OFFSET ${offset}
  `

  const result = await metrics.timer('ListDependentsDBLatencyMs', () =>
    pg.query(sql, values)
  )

  const rows = result.rows.map(({ _total, ...columns }) => ({ ...columns }))
  metrics.counter('ListDependentsRowsReturned', rows.length)
