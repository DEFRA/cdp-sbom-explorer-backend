...
describe('#search-queries', () => {
  describe('#build-query', () => {
    ...
    test('stage', () => {
      ...
      expect(query.sql).toEqual(`${baseSql} WHERE e.stage = $1`)
    ...
    test('environment', () => {
      ...
      expect(query.sql).toEqual(
        `${baseSql} ${deploymentsJoin} WHERE dpl.environment = $1`
      )
    ...
    test('name, version', () => {
      ...
      expect(query.sql).toEqual(
        `${baseSql} WHERE d.name = $1 AND d.version = $2`
      )
    ...
    test('name, version, type', () => {
      ...
      expect(query.sql).toEqual(
        `${baseSql} WHERE d.name = $1 AND d.version = $2 AND d.type = $3`
      )
    ...
    test('name, versionRange', () => {
      ...
      expect(query.sql).toEqual(
        `${baseSql} WHERE d.name = $1 AND d.version_num >= $2 AND d.version_num <= $3`
      )
    ...
    test('name, versionRange environment', () => {
      ...
      expect(query.sql).toEqual(
        `${baseSql} ${deploymentsJoin} WHERE d.name = $1 AND d.version_num >= $2 AND d.version_num <= $3 AND dpl.environment = $4`
      )
